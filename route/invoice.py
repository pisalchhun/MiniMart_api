...
def create_invoice():
    ...
    # Set total to 0 if not provided
...
def get_all_by_id(invoice_id):
    sql = text("SELECT * FROM invoice WHERE id = :invoice_id")
    result = db.session.execute(sql, {"invoice_id": invoice_id}).fetchone()
